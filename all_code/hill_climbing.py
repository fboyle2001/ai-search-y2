...
def hill_climb(tries_per_iter = 1000):
    current_solution = [x for x in range(num_cities)]
    #current_solution = [3, 1, 2, 7, 5, 11, 6, 0, 8, 9, 10, 4]
    ...
    while changed:
        ...
        dists_between_neighbours = calc_dist_neighbours(current_solution)

        print(dists_between_neighbours)

        for i in range(tries_per_iter):
            new_solution = mutate_state(current_solution, dists_between_neighbours)
