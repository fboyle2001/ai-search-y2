...
class Ant:
    def __init__(self, all_cities, start):
        self.all_cities = all_cities
        self.forbidden = set([start])
        self.path = []
        self.current = start

    def edge_probability(self, node, pheromones, hds, alpha, beta):
        return (pow(pheromones[self.current][node], alpha) * pow(hds[self.current][node], beta)) / self.sum_of_edge_values(pheromones, hds, alpha, beta)

    def sum_of_edge_values(self, pheromones, hds, alpha, beta):
        allowed = self.all_cities - self.forbidden
        s = 0

        for node in allowed:
            s += pow(pheromones[self.current][node], alpha) * pow(hds[self.current][node], beta)

        return s

    def make_move(self, pheromones, hds, alpha, beta):
        #can make so much more efficient by storing sum_of_edge_values
        possible_moves = self.all_cities - self.forbidden

        if(len(possible_moves) == 0):
            return False

        if(len(possible_moves) == 1):
            self.current = possible_moves.pop()
            self.path.append(self.current)
            self.forbidden.add(self.current)
            return True
        ...
        for node in possible_moves:
            cuml_prob += self.edge_probability(node, pheromones, hds, alpha, beta)
    ...
    def produce_tour(self, pheromones, hds, alpha, beta):
        self.path = []
        self.current = random.randint(0, len(self.all_cities) - 1)
        self.forbidden = set([self.current])

        while self.make_move(pheromones, hds, alpha, beta):
            pass
...
def ant_system_alg(n, tau_nought, max_it, initial_tour, rho, alpha, beta):
    ...
    ants = [Ant(set([x for x in range(num_cities)]), random.randint(0, num_cities - 1)) for _ in range(n)]

    for t in range(max_it):
        pheromone_additions = [[0 for _ in row] for row in pheromones]

        for ant in ants:
            new_tour = ant.produce_tour(pheromones, hds, alpha, beta)
            tl = tour_length_calc(new_tour)

            for index in range(0, len(new_tour) - 1):
                pheromone_additions[new_tour[index]][new_tour[index + 1]] += 1 / tl

            pheromone_additions[new_tour[len(new_tour) - 1]][new_tour[0]] += 1 / tl

            if tl < best_tour_length:
                best_tour_length = tl
                best_tour = new_tour

        next_iter_pheromones = [[v * (1 - rho) + pheromone_additions[ri][ci] for ci, v in enumerate(row)] for ri, row in enumerate(pheromones)]
        pheromones = next_iter_pheromones

    return best_tour

nn_tour = nearest_neighbour(0)
tau_nought = num_cities / tour_length_calc(nn_tour)
max_it = 1000
no_of_ants = num_cities
decay_rate = 0.5
alpha = 1
beta = 3

time_start = time.time()
tour = ant_system_alg(no_of_ants, tau_nought, max_it, nn_tour, decay_rate, alpha, beta)
