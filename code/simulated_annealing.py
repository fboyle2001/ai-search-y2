...
print("Number of cities is " + str(num_cities))
...
def mutate_state(state, dists_between_neighbours):
    mu_state = state[::]
    p = random.uniform(0, 1)

    if p < 0.4:
        #40% chance to swap 2 elements
        a = random.randrange(0, len(state))
        b = random.randrange(0, len(state))

        mu_state[a], mu_state[b] = mu_state[b], mu_state[a]
    elif 0.4 <= p < 0.65:
        #25% chance to force the end points to move
        a = random.randrange(1, len(state))
        b = random.randrange(0, len(state) - 1)

        mu_state[a], mu_state[0] = mu_state[0], mu_state[a]
        mu_state[b], mu_state[len(state) - 1] = mu_state[len(state) - 1], mu_state[b]
    else:
        #35% chance to swap largest neighbour pair
        pos = dists_between_neighbours.index(max(dists_between_neighbours))
        a = random.randrange(0, len(state))
        b = random.randrange(0, len(state))
        ...
        if pos == len(state) - 1:
            ...
            mu_state[a], mu_state[pos] = mu_state[pos], mu_state[a]
            mu_state[b], mu_state[pos + 1] = mu_state[pos + 1], mu_state[b]
...
def calc_dist_neighbours(state):
    return [dist_matrix[state[i]][state[i+1]] for i in range(0, len(state) - 1)] + [dist_matrix[state[0]][state[len(state) - 1]]]

def simulated_annealing(min_temp = 0.1):
    current_solution = [x for x in range(0, num_cities)]
    current_dist = tour_length_calc(current_solution)
    dist_neighbours = calc_dist_neighbours(current_solution)
    temperature = 3
    c = 0

    while temperature > min_temp:
        c+=1
        new_solution = mutate_state(current_solution, dist_neighbours)
        new_dist = tour_length_calc(new_solution)
        delta_dist = current_dist - new_dist
        ...
        if delta_dist >= 0:
            ...
            #delta_dist < 0 so 0 < math.exp(...) < 1
            prob_change = math.exp(delta_dist / temperature)
    ...
    print(c)
    return current_solution, current_dist

tour, tour_length = simulated_annealing()
...
if tour_length != check_tour_length:
    ...
    sys.exit()
print("Tour of length " + str(tour_length))
